...
public class DeleteEventCommandParser implements Parser<DeleteEventCommand> {
    ...
    public DeleteEventCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_EVENT_INDEX);
        ...
        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_EVENT_INDEX);

        eventIndex = ParserUtil.parseIndex(argMultimap.getValue(PREFIX_EVENT_INDEX).get());

        return new DeleteEventCommand(groupIndex, eventIndex);
    }
}
